...
#define FREQ1_CMP   1.561098E9          /* BeiDou B1I frequency (Hz) */
#define FREQ2_CMP   1.20714E9           /* BeiDou B2I frequency (Hz) */
#define FREQ3_CMP   1.26852E9           /* BeiDou B3I frequency (Hz) */
...
#ifndef NFREQ                           /* 注意：在预处理器中，NFREQ已被定义为 6 */
...
#define MAXPRNSBS   160                 /* max satellite PRN number of SBAS */
...
#ifndef MAXOBS
#define MAXOBS      80                  /* max number of obs in an epoch */
#endif
...
#else
#define DTTOL       0.025               /* tolerance of time difference (s)/时差容忍度 */
#endif
...
#define IONOOPT_BDSK8 11                 /* ionosphere option: BDS Klobuchar 8 parameters*/
#define IONOOPT_BDSSH9 12                /* ionosphere option: BDS SH9 parameters*/
#define IONOOPT_BDSION 13                /* ionosphere option: BDS SH9 parameters*/
#define IONOOPT_GALION 14                /* ionosphere option: Galileo niquick 3 parameters */
...
typedef struct {        /* time struct */
    time_t time;        /* time (s) expressed by standard time_t */
                        /* time_t结构体包含time，time表示参考时间至今的秒数 */
...
typedef struct {        /* observation data */
	...
	int isci[7][MAXFREQ]; /* record the ISC index: 0:pilot, 1:data */
} obs_t;
...
typedef struct {        /* antenna parameter type */
    ...
    double off[NFREQ][ 3]; /* phase center offset e/n/u or x/y/z (m) */ //相位中心偏移
...
typedef struct {        /* GPS/QZS/GAL broadcast ephemeris type */
    ...
    int sva;            /* SV accuracy (URA index)  SISMAI for BDS */
    int svh;            /* SV health (0:ok) */
    int week;           /* GPS/QZS: gps week, GAL: galileo week */
    int code;           /* GPS/QZS: code on L2C and L5 */
                        /* GAL: data source defined as rinex 3.03 */
                        /* BDS: data source (0:B1I/B2I/B3I,1:B1C,2:B2a,3:B2b) */
    int flag;           /* GPS/QZS: L2 P data flag */
                        /* BDS: nav type (0:unknown,1:GEO,2IGSO,3:MEO) */
    ...
    double tgd[MAXFREQ];      /* group delay parameters */
                        /* GPS/QZS:tgd[0]=TGD  */
                        /* GAL    :tgd[0]=BGD E5a/E1,tgd[1]=BGD E5b/E1 */
                        /* CMP    :tgd[0]=B1I,tgd[1]=B3I tgd[2]=B1C,tgd[3]=B2a,tgd[4]=B2b, tgd[5]=B2I  */
    double Adot,ndot;   /* Adot,ndot for CNAV */
	double sisai[4];   /* GPS :URAI_NED0,URAI_NED1,URAI_NE */
					   /* BDS :SISAI_oe,SISAI_ocb,SISAI_oc1,SISAI_oc2 */
	double isc[MAXFREQ];     /* GPS : ISC_L1CA ISC_L1Cd ISC_L1p ISC_L2C ISC_L5I5 ISCL5Q5 */
	                   /* BDS: B1I,B2I,B3I,ISC_B1Cd or ISC_B2ad */
	int intg_flag;       /*integrity flags */
	int ds;  /* data source */
} eph_t;
...
typedef struct {        /* GLONASS broadcast ephemeris type */
	...
	double tgd;         /* group delay parameters */
	int status;         /* Status Flags 9-bit binary number */
	int urai;           /*  GLO-M/K only – raw accuracy index FT */
	int hflag;             /* Health Flags s 3-bit binary number*/
} geph_t;
...
typedef struct {        /* navigation data type */
    ...
    erp_t  erp;         /* earth rotation parameters / 地球自传参数 */
	...
	BDSSH  *ion_bdsk9;   /* BeiDou iono model parameters k9*/
	double tgd[MAXSAT][NFREQ]; /* tgd (s),B1I/2I/3I/1C/2a */
	int galfreq, galcode;   /* */
	int obstsys;
	int igmasta;
	int isci[7][MAXFREQ]; /* record the ISC index: 0:pilot, 1:data */
...
typedef struct {        /* solution type */
	...
	double dop[6];       /* gdop/pdop/hdop/vdop/tdop/rdop*/
	double rf[3];        /* user precise positions */
	int sat[MAXSAT];
	int obstsys;
} sol_t;
...
//    int freqopt;        /* disable L2-AR */
	int freqopt;        /* 32 bit, f1->bit0=1,f2->bit1=1,f3->bit2=1，f4->bit3=1，f5->bit4=1 */
    char pppopt[256];   /* ppp option */
	double  coordfixed;      /* nalysis only.0: SPP, unlimited~1E6: fixed to known position. Default: 0 */
	int  outsat;
} prcopt_t;
...
typedef struct {        /* solution options type */
	...
	int  igmasfmt;       /* solution output format (0:off,1:on) */
	int outsat;
	int navsys;
} solopt_t;

typedef struct {        /* file options type */
    char satantp[MAXSTRPATH]; /* satellite antenna parameters file */   //卫星天线参数文件
...
typedef struct {        /* satellite status type */
	...
	double  phr[2][NFREQ]; /* previous carrier-phase observable (cycle) */
	double  pha[2][NFREQ]; /* previous carrier-phase observable (cycle) */
	unsigned char outcflag[NFREQ]; /*  satellite outage flag */
} ssat_t;
...
EXPORT void traceobs(int level, const obsd_t *obs, int n, prcopt_t opt);
...
EXPORT double geodist(int sys, const double *rs, const double *rr, double *e, double *dr);
...
EXPORT int ionocorr(prcopt_t opt,gtime_t time, const nav_t *nav, int sat, const double *pos,const double *satpos,
...
EXPORT int lexioncorr(gtime_t time, const nav_t *nav, const double *pos,
                      const double *azel, double *delay, double *var);
extern double ionmodel_BDSK8(gtime_t t, const double *ion, const double *pos, const double *azel);
extern double ionmodel_BDSK9(gtime_t t, const BDSSH *bdssh, const double *pos, const double *azel);
...
extern double median2_t(double *data, int n);
