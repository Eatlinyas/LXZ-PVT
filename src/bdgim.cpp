...
{
	double IMF=0.0;
	double RE = 6378E3;	 // the average radius of earth [unit: m]
	...
	case 3:
		...
		break;
	case 4:
		// BDSSH  mapping function
		IMF = 1 / sqrt(1 - pow(RE*cos(sat_elev) / (RE + Hion), 2.0));
		break;
	default:
...
*****************************************************************************/
extern int IonBdsBrdModel(NonBrdIonData* nonBrdData, BrdIonData* brdData, double mjd, double* sta_xyz, double* sat_xyz, double* brdPara, double* ion_delay) {
