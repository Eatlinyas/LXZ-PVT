...
//#define FRQOPT  "1:l1,2:l1+l2,3:l1+l2+l5,4:l1+l5,5:l1+l2+l5+l6+l7"
/* 
1-2-4-8-16-32:  l1/l2/l5/l6/l7/l8
3: l1+l2 GPS L1+2, BDS B1I+2I, GAL E1+5b, GLO L1+2
5: l1+l5 GPS L1+5, BDS B1I+B3I, GAL E1+E5a
9: l1+l6 GAL E1+E6
24: l6+l7 BDS B1C+2a
40: l6+l8 BDS B1C+2b, GAL E6+E5
*/
#define FRQOPT  "1:l1,2:l2,4:l5,8:l6,16:l7,32:l8,3:l1+l2,5:l1+l5,9:l1+l6,24:l6+l7,40:l6+l8"

#define TYPOPT  "0:forward,1:backward,2:combined"
#define IONOPT  "0:off,1:brdc,2:sbas,3:dual-freq,4:est-stec,5:ionex-tec,6:qzs-brdc,7:qzs-lex,8:vtec_sf,9:vtec_ef,10:gtec,11:bdsk8,12:bdssh9,13:bdsion,14:galion"
...
#define IGMASOPT "0:off,1:on"
#define SATOPT   "0:off,1:on"
...
EXPORT opt_t sysopts[]={
	...
	{"coordinate-fixed",1,  (void *)&prcopt_.coordfixed,  "" },
	...
	{"out-outsat",      3,  (void *)&prcopt_.outsat, SATOPT },
...
截断注释：删除 # 及其后的所有内容（配置文件中的注释）。
修剪尾部空白：删除字符串末尾的所有不可见字符（如空格、换行符等）。*/
...
extern int loadopts(const char *file, opt_t *opts)
{
    ...
    while (fgets(buff,sizeof(buff),fp)) 
    {
        ...
        *p++='\0';
        chop(buff);
        //opt（sysopts）是一个含有很多设置选项（opt）的数组
        //通过searchopt函数在opt中查找buff（选项名）
        //返回值是一个指向对应opt的指针
        //再通过str2opt函数将buff（选项值）转换为对应的选项值
...
/*  redesign freq info. according to freq no. */
extern int test_freq(int freq0,int  ifreq){
	switch (freq0){
	case 1: return ifreq==0;
	case 2: return ifreq==1;
	case 4: return ifreq==2;
	case 8: return ifreq==3;
	case 16: return ifreq==4;
	case 32: return ifreq == 5;
	case 3: return ifreq == 0 || ifreq == 1;
	case 5: return ifreq == 0 || ifreq == 2;
	case 9: return ifreq == 0 || ifreq == 3;
	case 24: return ifreq == 3 || ifreq == 4;
	case 40: return ifreq == 3 || ifreq == 5;
	default:  return 0;
	}
}
...
static void buff2sysopts(void)
{
    ...
    if (exsats_[0]!='\0') {
		...
		for (p = strtok(buff, " "); p; p = strtok(NULL, " ")) {
			...
			if ((q = strchr(id, '-'))){
				...
				sscanf(id, "%*[^-]-%s", buff2);
				sat2 = satid2no(buff2);
				for (i = sat; i <= sat2; i++)prcopt_.exsats[i - 1] = 1;
			}
			else if ((sat = satid2no(id))) {
    ...
    //    prcopt_.freqopt=1;
    //}
	prcopt_.freqopt = prcopt_.nf;

	prcopt_.nf=redesign_freq(prcopt_.nf);
}
