                   ...
                   const solopt_t *opt)
{
	...
	p += sprintf(p, "%s%s%14.4f%s%14.4f%s%14.4f%s%14.4f%s%14.4f%s%14.4f%s %14.4f%s%14.4f%s%14.4f%s%8.2f%s%3d%s",
		s, sep, sol->rr[0], sep, sol->rr[1], sep, sol->rr[2], sep, enu[0], sep, enu[1], sep, enu[2], sep,
		sol->rr[3], sep, sol->rr[4], sep, sol->rr[5], sep, sol->dop[1], sep, sol->ns, sep
		);
	for (i = 0; i < MAXSAT; i++) {
		sys = satsys(i + 1, &prn);
		if (sys != opt->navsys)continue;
		if (sol->sat[i] == -1)prn = -prn;
		else if (sol->sat[i] == 0)prn = 0;
		p += sprintf(p, "%4d", prn);
	}
	p += sprintf(p, "\n");
	return p - (char *)buff;
...
void str_freq(int nfreq, char *str){

	switch (nfreq){
	case 1:  strcpy(str, "L1");return ;
	case 2:  strcpy(str, "L2"); return;
	case 4:  strcpy(str, "L5"); return;
	case 8:  strcpy(str, "L6"); return;
	case 16: strcpy(str, "L7"); return;
	case 32: strcpy(str, "L8"); return;
	case 3:  strcpy(str, "L1+l2"); return;
	case 5:  strcpy(str, "L1+L5"); return;
	case 9:   strcpy(str, "L1+L6"); return;
	case 24:  strcpy(str, "L6+L7"); return;
	case 40:  strcpy(str, "L6+L8"); return;
	default: printf("unknown frequnce code! %d\n", nfreq);   strcpy(str, "UNKNOWN OBS_TYPE!"); return;
	}
}
...
extern int outprcopts(unsigned char *buff, const prcopt_t *opt)
{
    ...
    const char *s4[]={"off","broadcast","sbas","iono-free","estimation",
		"ionex tec", "qzs", "lex", "vtec_sf", "vtec_ef", "gtec", "bdsk8", "bdssh9", "bdsion", "galion" };
	...
	str_freq(opt->freqopt, str_freqopt);
	...
	p += sprintf(p, "%s frequency : %s", COMMENTH, str_freqopt);
	p += sprintf(p, "\n");
...
extern int outsolheads(unsigned char *buff, const solopt_t *opt)
{
	...
	const char *s3[] = { "BDT", "UTC ", "JST " }, *sep = opt2sep(opt);
    char *p=(char *)buff;
    ...
    if (opt->posf==SOLF_LLH) { /* lat/lon/hgt */
		...
		if (opt->igmasfmt)
		{
			p += sprintf(p, "%14s%s%14s%s%14s%s%14s%s%14s%s%14s%s %14s%s%14s%s%14s%s%8s%s%3s%s%s",
				"x-ecef(m)", sep, "y-ecef(m)", sep, "z-ecef(m)", sep, "e", sep, "n", sep, "u", sep,
				"ve", sep, "vn", sep, "vu", sep, "pdop", sep,
				"ns", sep, "C01 C02 C03 C04 C05...  C67 (svn and svh(positive: health, otherwise; negative. E.G. -5))");
		}
		else
		{
		...
		if (opt->igmasfmt)
		{
			...
			p += sprintf(p, "%8s%s%8s%s%15s%s%14s%s%14s%s%14s%s%14s%s%3s%s%3s%s%8s%s%14s%s%14s%s%14s%s%14s%s%14s%s%14s%s%14s",
				"mjd(d)", sep, "sod(s)", sep, "fjd(d)", sep, "e-baseline(m)", sep, "n-baseline(m)", sep, "u-baseline(m)", sep,
				"3D-baseline(m)", sep, "Q", sep, "ns", sep, "ratio", sep,
				"rove_x(m)", sep, "rove_y(m)", sep, "rove_z(m)", sep, "base_x(m)", sep, "base_y(m)", sep, "base_z(m)", sep, "pdop");
		}
    }
//	p += sprintf(p, "%s%s%14.4f%s%14.4f%s%14.4f%s%14.4f%s%14.4f%s%3d%s%3d%s%8.4f\n",
