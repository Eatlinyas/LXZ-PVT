...
static const double gpst0[]={1980,1, 6,0,0,0}; /* gps time reference            /GPS参考时间 */
static const double gst0 []={1999,8,22,0,0,0}; /* galileo system time reference /GLONASS参考时间 */
static const double bdt0 []={2006,1, 1,0,0,0}; /* beidou time reference         /BDS参考时间 */
...
static char *obscodes[]={       /* observation code strings */
    ...
    "5B","5C","9A","9B","9C", "9X","1D","5D","5P","8I",  /* 50-59 */
	 "7D", "7P", "7Z", "", "", "", "", "", "", ""        /* 60-69 */
};
static unsigned char obsfreqs[]={
    ...
    3, 3, 7, 7, 7,  7, 1, 3, 3, 6,  /* 50-59 */
	6, 6, 6, 0, 0, 0, 0, 0, 0, 0    /* 60-69 */
};
...
static char codepris[7][MAXFREQ][16] = {  /* code priority table */
	...
	/* L1/E1        L2/B1        L5/E5a/L3 L6/LEX/B3 E5b/B2    E5(a+b)  S */
	/* B1C          B1I          B2a       B3I       B2I       B2b    B2a+2b */
	{ "CPYWMNSL", "PYWCMNDSLX", "IQX",      "",      "",       "",      ""   }, /* GPS */
	{ "PC",       "PC",         "IQX",      "",      "",       "",      ""   }, /* GLO */
	{ "CABXZ",    "",           "IQX",      "ABCXZ", "IQX",    "IQX",   ""   }, /* GAL */
	{ "CSLXZ",    "SLX",        "IQX",      "SLX",   "",       "",      ""   }, /* QZS */
	{ "C",        "",           "IQX",       "",     "",       "",      ""   }, /* SBS */
	{"PXBCXZQDAI", "IQX",        "PXQDI",    "IQX",  "IQX",   "PCZDA",  "PXQID" }, /* BDS */
	{ "",         "",           "ABCX",      "",     "",       "",    "ABCX" }  /* IRN */
}; 
...
* 将观测类型字符串（obs type string）转换为观测码（obs code）
* 将字符("1C","1P","1Y",...)，转换为观测码(具体见obscodes定义)
...
extern int getcodepri(int sys, unsigned char code, const char *opt)
{
    ...
    //j为返回的该信号的频率代号，例如：1代表L1频段，1575.42MHz
    //obs为信号种类（1X，2I……）
    obs=code2obs(code,&j);  
    ...
    for (p=opt;p&&(p=strchr(p,'-'));p++) {
        //将p中的内容按optstr的格式读入到str中，即str为-?L后面的最多两个非空白字符
...
/* multiply matrix 矩阵乘法 C = (a * A * B) + (b * C) -----------------------------------------------------------
    tr表示乘法格式，N代表无改动，T代表转置，以此类推：NT代表A矩阵无改动，B矩阵转置
    A矩阵n行m列，B矩阵m行k列，C矩阵n行k列
    a(alpha)代表矩阵前面乘的常数，b(beta)代表C矩阵前乘的常数*/
extern void matmul(const char *tr, int n, int k, int m, double alpha,
                   const double *A, const double *B, double beta, double *C)
{
    double d;
    ...
    for (i=0;i<n;i++) 
        for (j=0;j<k;j++) 
        {
            ...
            switch (f) {
                // C矩阵i行j列的元素计算
                // 注：序号编排以列为基准，当前序号为：之前列元素（左侧列）之和+该元素在该列的序号（行号）
...
extern int lsq(const double *A, const double *y, int n, int m, double *x,
               double *Q,double coordfixedvalue)
{
    double *Ay;
    int i,info;
	...
	for (i = 0; i<3; i++) Q[i + i*n] += coordfixedvalue*coordfixedvalue;
...
* 指针向后移i个位置，从第i+1个字符开始，提取n个字符并转换为double
* 示例：对于buff="123456789",str2num(buff,3,3)="456"
...
extern double str2num(const char *s, int i, int n)
{
    ...
    if (i<0||(int)strlen(s)<i||(int)sizeof(str)-1<n) return 0.0;    //起始位置不能为负，不能超出s长度，提取数目不能超过str长度
...
* gtime转数组储存时间，ep中的元素分别为：year,month,day,hour,min,sec
...
* gtime_t转GPS time（周+周内秒的形式），week为周，使用指针修改，返回周内秒（double）
* args   : gtime_t t        I   gtime_t struct
...
* read earth rotation parameters / 读取地球自转参数
...
static int cmpeph(const void *p1, const void *p2)
{
    ...
    return q1->ttr.time!=q2->ttr.time?(int)(q1->ttr.time-q2->ttr.time):
		   ...
		   (q1->code != q2->code ? (int)(q1->code - q2->code) :
            q1->sat-q2->sat)));
}

static int isys(int sys, int code, int ifrq)
{
	int k = -1, n = 0;
	switch (sys) {
	case SYS_GPS: k = 0; n = 1;  break;
	case SYS_GLO: k = 1; n = 1; break;
	case SYS_GAL: k = 2; n = 1; break;
	case SYS_QZS: k = 3; n = 1; break;
...
static void uniqeph(nav_t *nav)
{
    ...
    for (i=1,j=0;i<nav->n;i++) {
        ...
        if (nav->eph[i].sat!=nav->eph[j].sat||
			...
			nav->eph[i].ttr.time!=nav->eph[j].ttr.time) {
            nav->eph[++j]=nav->eph[i];
        }
    }
    nav->n=j+1;
    ...
    trace(4,"uniqeph: n=%d\n",nav->n);
	trace(4, "Tgd from ephs: \n");
	for (i = 0; i < MAXSAT; i++) 
    {
		for (j = 0; j < MAXFREQ; j++)
            tgd[j] = 0.0;
		if (satsys(i + 1, &prn) != SYS_CMP && satsys(i + 1, &prn) != SYS_GAL)
            continue;
		for (int ifrq = 0; ifrq < MAXFREQ; ifrq++)
        {
			tgd[ifrq] = nav->tgd[i][ifrq];
		}
		satno2id(i+1, cprn);
		trace(4, "sat=%s: %19.10E %19.10E %19.10E %19.10E %19.10E\n", cprn, tgd[0], tgd[1], tgd[2], tgd[3], tgd[4]);
	}
}
...
* return : number of epochs 返回观测文件历元数n
*-----------------------------------------------------------------------------*/
extern int sortobs(obs_t *obs)
{
    ...
    for (i=n=0;i<obs->n;i=j,n++) {
        for (j=i+1;j<obs->n;j++) {
            if (timediff(obs->data[j].time,obs->data[i].time)>DTTOL) //若大于DTTOL，则下个观测值不再是同一个历元的，故停止
                break;
        }
    }
    //返回观测文件历元数n
...
extern void traceopen(const char *file)
{
    ...
    if (!*path||!(fp_trace=fopen(path,"w"))) fp_trace=stderr;   //如果路径不存在或者无法以写入格式打开，则fp_trace为stderr（标准错误流）
    strcpy(file_trace,file);
    tick_trace=tickget();   //获取当前时间（精确到ms）
    time_trace=time;
    initlock(&lock_trace);  //多线程同步用
}
//如果fp_trace有效且fp_trace不为stderr（标准错误流），关闭fp_trace，恢复初始值，清空指针和路径
...
extern void traceobs(int level, const obsd_t *obs, int n, prcopt_t opt)
{
    char str[64],id[16];
    int i,f;
	int fidx[2];
	...
	fidx[0] = fidx[1] = -1;
	for (i = 0,f=0; i<MAXFREQ; i++){
		if (test_freq(opt.freqopt, i)){
			fidx[f++] = i;
		}
	}
	fidx[0] = fidx[0]<0?0:fidx[0];
	fidx[1] = fidx[1]<0 ? (fidx[0] == MAXFREQ - 1 ? fidx[0] - 1 : fidx[0] + 1) : fidx[1];
    if (!fp_trace||level>level_trace) return;
    for (i=0;i<n;i++) {
        time2str(obs[i].time,str,3);
        satno2id(obs[i].sat,id);
        fprintf(fp_trace," (%2d) %s %-3s rcv%d %13.3f %13.3f %13.3f %13.3f %d %d %d %d %3.1f %3.1f\n",
			i + 1, str, id, obs[i].rcv, obs[i].L[fidx[0]], obs[i].L[fidx[1]], obs[i].P[fidx[0]],
			obs[i].P[fidx[1]], obs[i].LLI[fidx[0]], obs[i].LLI[fidx[1]], obs[i].code[fidx[0]],
			obs[i].code[fidx[1]], obs[i].SNR[fidx[0]] * 0.25, obs[i].SNR[fidx[1]] * 0.25);
    }
    fflush(fp_trace);
}
...
/* expand file path ------------------------------------------------------------
* expand file path with wild-card (*) in file / 查找符合通配符规则的文件，并按字母顺序排列，返回找到的文件数目
...
/* replace string ------------------------------------------------------------*/
/* str为文件指针，pat为要修改的关键字，rep为添加的内容，如果没找到关键字会什么都不做 */
static int repstr(char *str, const char *pat, const char *rep)
{
    ...
    for (p=str,r=buff;*p;p=q+len) 
    {
        ...
        strncpy(r,p,q-p); //从p中拷贝q-p个字符到r中，如果p中字符不足，会提前填充'\0'
        r+=q-p; //r向后q-p个字符
        r+=sprintf(r,"%s",rep); //从此处开始，写入rep内容
    }
    if (p<=str) return 0;
    strcpy(r,p);    //将p内容复制到r
    strcpy(str,buff);
...
* 把path复制到rpath上，在rpath上进行修改，主要是修改时间表达
* 示例：path = "data/%Y/%m/%d/%r-%b_%h%M.obs"
* 修改后：rpath = "data/2024/03/13/RoverA-BaseB_1530.obs"
...
* return : status (1:keywords replaced, 
*                  0:no valid keyword in the path,
*                  -1:no valid time)
                   ...
                   const char *base)
{
    ...
    if (time.time!=0) {
        time2epoch(time,ep);    //gtime转数组储存时间，ep中的元素分别为：year,month,day,hour,min,sec
        ep0[0]=ep[0];
        dow=(int)floor(time2gpst(time,&week)/86400.0);      //dow为 day of week 表示一周中的第几天
        doy=(int)floor(timediff(time,epoch2time(ep0))/86400.0)+1;   //年积日
...
extern double satwavelen(int sat, int frq, const nav_t *nav)
{
    ...
    if (sys==SYS_GLO) {
        ...
        else if (frq==1) return CLIGHT/FREQ2_CMP; /* B2I */
        else if (frq==2) return CLIGHT/FREQ3_CMP; /* B3I */
		else if (frq==3) return CLIGHT/FREQ1; /* B1C */
		else if (frq==4) return CLIGHT/FREQ5; /* B2a */
		else if (frq==5) return CLIGHT/FREQ7; /* B2b */
    }
    else if (sys==SYS_GAL) {
...
*-----------------------------------------------------------------------------*/
extern double geodist(int sys, const double *rs, const double *rr, double *e, double *dr)
{
	double r;
	int i;
	double omiga, dsec = 0;
	...
	case SYS_GPS:
		omiga = OMGE; break;
	case SYS_GLO:
		omiga = OMGE_GLO; break;
	case SYS_GAL:
		omiga = OMGE_GAL; break;
	case SYS_QZS:
		omiga = OMGE; break;
	case SYS_CMP:
		omiga = OMGE_CMP; break;
	case SYS_SBS:
		omiga = OMGE; break;
	default: omiga = OMGE;
	}

	if (dr)*dr = omiga*(rs[0] * rr[1] - rs[1] * rr[0]) / CLIGHT;

	return r + omiga*(rs[0] * rr[1] - rs[1] * rr[0]) / CLIGHT;
                       ...
                       const double *azel)
{
	...
	/* GPS L1 -> BDS B1I*/
	k = FREQ1*FREQ1 / FREQ1_CMP / FREQ1_CMP;
	return dion*k;
}
                        ...
                        double humi)
{
	...
	trph = 0.0022768*(pres - tan(z)*tan(z)) / cos(z);
