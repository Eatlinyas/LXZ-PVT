...
static void outsolstat(rtk_t *rtk)
{
    ...
    for (i=0;i<MAXSAT;i++) {
        ...
        satno2id(i+1,id);
        for (j=0;j<nfreq;j++) {
			if (fidx[j]<0)continue;
            fprintf(fp_stat,"$SAT,%d,%.3f,%s,%d,%.1f,%.1f,%.4f,%.4f,%d,%.0f,%d,%d,%d,%d,%d,%d,%8.3f\n",
                    week,tow,id,j+1,ssat->azel[0]*R2D,ssat->azel[1]*R2D,
                    ssat->resp[j],ssat->resc[j],ssat->vsat[j],ssat->snr[j]*0.25,
                    ssat->fix[j],ssat->slip[j]&3,ssat->lock[j],ssat->outc[j],
                    ssat->slipc[j],ssat->rejc[j],rtk->sol.ratio);
        }
    }
}
/* save error message --------------------------------------------------------*/
...
static double gfobs_LiLj(const obsd_t *obs, int i, int j, int fi,int fj,const double *lam)
{
	double pi = sdobs(obs, i, j, fi)*lam[fi], pj = sdobs(obs, i, j, fj)*lam[fj];
	return pi == 0.0 || pj == 0.0 ? 0.0 : pi - pj;
}
...
/* for single frequency relative positioning, extended to two freqs as soon as possible. */
extern int guess_frqidx(int sat, prcopt_t opt, int *f_idx)
{
	int k,m, nf = 0;
	int sys0[4] = { 1, 4, 8, 32 };/*GPS/GLO/GAL/BDS*/
	int freq0[4] = {3,3,3,5},freq=0;
	int fidx[MAXFREQ],sys=satsys(sat,NULL);

	for (m = 0; m < 4; m++) /* m=0:gps/sbs,1:glo,2:gal,3:bds,4:qzs */
	{
		if (!test_sys(sys, m)) continue;
		if (sys == SYS_CMP){ freq = (opt.freqopt != 8 && opt.freqopt != 16) ? freq0[m] : 24; }
		if (sys == SYS_GPS){ freq = (opt.freqopt != 1 && opt.freqopt != 2) ? freq0[m] : 5; }
		if (sys == SYS_GAL){ freq = (opt.freqopt != 1 && opt.freqopt != 2) ? freq0[m] : 5; }
		for (k = 0; k < MAXFREQ; k++)
		{
			test_freq(freq, k) ? (f_idx[nf++] = k) : 0; 
		}
		return nf;
	}
}

/* detect cycle slip by Li?-Lj? geometry free phase jump -----------------------*/
static void detslp_gf_LiLj(rtk_t *rtk, const obsd_t *obs, int i, int j,
	const nav_t *nav)
{
	int sat = obs[i].sat;
	double g0, g1;
	...
	frqidx(opt, fidx);
	if (frqidx(opt, fidx) < 2) {
		if(guess_frqidx(obs->sat, opt, fidx)<2) return;
	}
	if ((g1 = gfobs_LiLj(obs, i, j, fidx[0], fidx[1], nav->lam[sat - 1])) == 0.0) return;

	g0 = rtk->ssat[sat - 1].gf; rtk->ssat[sat - 1].gf = g1;

	//if(sat==123)printf("%lf %lf \n", g0, g1);

	if (g0 != 0.0&&fabs(g1 - g0)>rtk->opt.thresslip) {

		rtk->ssat[sat - 1].slip[fidx[0]] |= 1;
		rtk->ssat[sat - 1].slip[fidx[1]] |= 1;

		errmsg(rtk, "slip detected GF_jump (sat=%2d GF_L%d_L%d=%.3f %.3f)\n", sat, fidx[0], fidx[1], g0, g1);
	}
}
                       ...
                       const nav_t *nav)
{
...
#if 1
    int f,sat=obs[i].sat;
    double tt,dph,dpt,phr,pha,lam,thres;
	int fidx[MAXFREQ];
	...
	frqidx(rtk->opt, fidx);
    for (f=0;f<rtk->opt.nf;f++) {
		if (obs[i].L[f] == 0.0 || obs[i].D[f] == 0.0 || rtk->ssat[obs[i].sat - 1].ph[obs[i].rcv - 1][fidx[f]]== 0.0) {
            continue;
        }
		if (fabs(tt = timediff(obs[i].time, rtk->ssat[obs[i].sat - 1].pt[obs[i].rcv - 1][fidx[f]]))<DTTOL) continue;
		if ((lam = nav->lam[sat - 1][fidx[f]]) <= 0.0) continue;
        
        /* cycle slip threshold (cycle) */
        thres=rtk->opt.err[4]*fabs(tt)*4.0;
        
        /* phase difference and doppler x time (cycle) */
		dph = obs[i].L[f] - rtk->ssat[obs[i].sat - 1].ph[obs[i].rcv - 1][fidx[f]];
        dpt=-obs[i].D[f]*tt;
		phr = rtk->ssat[obs[i].sat - 1].phr[obs[i].rcv - 1][fidx[f]] == 0.0 ? 0.0: rtk->ssat[obs[i].sat - 1].phr[obs[i].rcv - 1][fidx[f]];
		pha = rtk->ssat[obs[i].sat - 1].pha[obs[i].rcv - 1][fidx[f]] == 0.0 ? 0.0 : rtk->ssat[obs[i].sat - 1].pha[obs[i].rcv - 1][fidx[f]];

		rtk->ssat[obs[i].sat - 1].pha[obs[i].rcv - 1][fidx[f]] = (dph/tt-rtk->ssat[obs[i].sat - 1].phr[obs[i].rcv - 1][fidx[f]])/tt;
		rtk->ssat[obs[i].sat - 1].phr[obs[i].rcv - 1][fidx[f]] = dph / tt;

		//printf("%d %lf %lf %lf\n", obs[i].sat, dph, dph-dpt,phr*tt+0.5*(pha)*tt*tt-dpt);
        if (fabs(dph-dpt)<=thres) continue;
        
        rtk->ssat[sat - 1].slip[fidx[1]]|=1;
        
        errmsg(rtk,"slip detected by dop(sat=%2d rcv=%d L%d=%.3f %.3f thres=%.3f)\n",
               sat,rcv,f+1,dph,dpt,thres);
		trace(2, "slip detected by dop(sat=%2d rcv=%d L%d=%.3f %.3f thres=%.3f)\n",
			sat, rcv, f + 1, dph, dpt, thres);
    }
#endif
}
                   ...
                   const int *iu, const int *ir, int ns, const nav_t *nav)
{
    ...
    for (i=0;i<ns;i++) {
        ...
        /* detect cycle slip by LLI */
		for (f = 0; f<rtk->opt.nf; f++) rtk->ssat[sat[i] - 1].slip[fidx[f]] &= 0xFC;
       ...
       // detslp_gf_L1L5(rtk,obs,iu[i],ir[i],nav);
		detslp_gf_LiLj(rtk, obs, iu[i], ir[i], nav);
       ...
       // detslp_dop(rtk,obs,ir[i],2,nav);
        ...
        for (f=0;f<nf;f++) {
			rtk->ssat[sat[i] - 1].half[fidx[f]] =
				!((obs[iu[i]].LLI[fidx[f]] & 2) || (obs[ir[i]].LLI[fidx[f]] & 2));
        }
    }
    for (f=0;f<nf;f++) {
        ...
        for (i=1;i<=MAXSAT;i++) {
			...
			reset = ++rtk->ssat[i - 1].outc[fidx[f]]>(unsigned int)rtk->opt.maxout;
			if (rtk->x[IB(i, f, &rtk->opt)] != 0.0)trace(3, "sat=%d outc no.=%d %d\n", i, rtk->ssat[i - 1].outc[fidx[f]], rtk->opt.maxout);
            if (rtk->opt.modear==ARMODE_INST&&rtk->x[IB(i,f,&rtk->opt)]!=0.0) {
                ...
                trace(3,"udbias : obs outage counter overflow (sat=%3d L%d n=%d)\n",
					i, f + 1, rtk->ssat[i - 1].outc[fidx[f]]);
				rtk->ssat[i - 1].outc[fidx[f]] = 0;
            }
            if (rtk->opt.modear!=ARMODE_INST&&reset) {
				rtk->ssat[i - 1].lock[fidx[f]] = -rtk->opt.minlock;
            }
        }
        ...
        for (i=0;i<ns;i++) {
			...
			slip = rtk->ssat[sat[i] - 1].slip[fidx[f]];
			if (rtk->opt.ionoopt == IONOOPT_IFLC) slip |= rtk->ssat[sat[i] - 1].slip[fidx[1]];
            if (rtk->opt.modear==ARMODE_INST||!(slip&1)) continue;
            rtk->x[j]=0.0;
			rtk->ssat[sat[i] - 1].lock[fidx[f]] = -rtk->opt.minlock;
        }
        bias=zeros(ns,1);
        ...
        for (i=j=0,offset=0.0;i<ns;i++) {
            ...
            if (rtk->opt.ionoopt!=IONOOPT_IFLC) {
				cp = sdobs(obs, iu[i], ir[i], fidx[f]); /* cycle */
				pr = sdobs(obs, iu[i], ir[i], fidx[f] + NFREQ);
				lami = nav->lam[sat[i] - 1][fidx[f]];
                if (cp==0.0||pr==0.0||lami<=0.0) continue;
                
                bias[i]=cp-pr/lami;
            }
            else {
				if (fidx[0] < 0 || fidx[1] < 0) continue;

				cp1 = sdobs(obs, iu[i], ir[i], fidx[0]);
				cp2 = sdobs(obs, iu[i], ir[i], fidx[1]);
				pr1 = sdobs(obs, iu[i], ir[i], NFREQ + fidx[0]);
				pr2 = sdobs(obs, iu[i], ir[i], NFREQ + fidx[1]);
				lam1 = nav->lam[sat[i] - 1][fidx[0]];
				lam2 = nav->lam[sat[i] - 1][fidx[1]];
                      ...
                      const prcopt_t *opt, double *y)
{
    ...
    if (opt->ionoopt==IONOOPT_IFLC) { /* iono-free linear combination */
		for (i = 0,fi=-1,fj=-1; i < opt->nf; i++) {
			fi = fi < 0 && test_freq(opt->freqopt, i) ? i : fi;
			fj = fi >= 0 && fi!=i&&test_freq(opt->freqopt, i) ? i : fj;
		}
		if (fi<0||fj<0) return;
        if (lam[fi]==0.0||lam[fj]==0.0) return;
        
        if (testsnr(base,fi,azel[1],obs->SNR[fi]*0.25,&opt->snrmask)||
            testsnr(base,fj,azel[1],obs->SNR[fj]*0.25,&opt->snrmask)) return;
        
        f1=CLIGHT/lam[fi];
        f2=CLIGHT/lam[fj];
        C1= SQR(f1)/(SQR(f1)-SQR(f2));
        C2=-SQR(f2)/(SQR(f1)-SQR(f2));
        dant_if=C1*dant[fi]+C2*dant[fj];
        
        if (obs->L[fi]!=0.0&&obs->L[fj]!=0.0) {
            y[0]=C1*obs->L[fi]*lam[fi]+C2*obs->L[fj]*lam[fj]-r-dant_if;
        }
        if (obs->P[fi]!=0.0&&obs->P[fj]!=0.0) {
            y[1]=C1*obs->P[fi]+C2*obs->P[fj]-r-dant_if;
        }
    }
    else {
        for (i=0;i<nf;i++) {
                 ...
                 int index, double *y, double *e, double *azel)
{
    ...
    for (i=0;i<n;i++) {
        ...
        if ((r=geodist(sys,rs+i*6,rr_,e+i*3,NULL))<=0.0) continue;
...
//	int k, fi, fj;
//
//	for (k = 0; k < MAXFREQ; k++) f_idx[k] = 0;
//	for (k = 0, fi = -1, fj = -1; k < opt.nf; k++) {
//		fi = fi < 0 && test_freq(opt.freqopt, k) ? k : fi;
//		fj = fi >= 0 && fi != k&& test_freq(opt.freqopt, k) ? k : fj;
//	}
...
//		for (k = 0; k < opt.nf; k++)
//		{
//			f_idx[k] = test_freq(opt.freqopt, k) ? k : 0;
//		}
//	}
//}
...
extern int frqidx(prcopt_t opt, int *f_idx)
{
	...
	for (k = 0; k < opt.nf; k++)
	{
		if (opt.ionoopt == IONOOPT_IFLC){ test_freq(opt.freqopt, k) ? (f_idx[nf++] = k) : 0; }
		else {f_idx[k] = k; nf++;}
	}
	return nf;
}
                 ...
                 double *H, double *R, int *vflg)
{
    ...
    for (f=opt->mode>PMODE_DGPS?0:nf;f<nf*2;f++) {
        ...
        for (j=0;j<ns;j++) {
            ...
            ff=f%nf;
			lami = nav->lam[sat[i] - 1][fidx[ff]];
			lamj = nav->lam[sat[j] - 1][fidx[ff]];
            ...
            if (f<nf) rtk->ssat[sat[j]-1].resc[fidx[f]   ]=v[nv];
            else      rtk->ssat[sat[j]-1].resp[fidx[f-nf]]=v[nv];
            
            /* test innovation */
            if (opt->maxinno>0.0&&fabs(v[nv])>opt->maxinno) {
                if (f<nf) {
					rtk->ssat[sat[i] - 1].rejc[fidx[f]]++;
					rtk->ssat[sat[j] - 1].rejc[fidx[f]]++;
                }
                errmsg(rtk,"outlier rejected (sat=%3d-%3d %s%d v=%.3f)\n",
                       sat[i],sat[j],f<nf?"L":"P",fidx[f%nf+1],v[nv]);
                continue;
            }
            ...
            if (opt->mode>PMODE_DGPS) {
				if (f<nf) rtk->ssat[sat[i] - 1].vsat[fidx[f]] = rtk->ssat[sat[j] - 1].vsat[fidx[f]] = 1;
            }
            else {
				rtk->ssat[sat[i] - 1].vsat[fidx[f - nf]] = rtk->ssat[sat[j] - 1].vsat[fidx[f-nf]] = 1;
            }
            trace(4,"sat=%3d-%3d %s%d v=%13.3f R=%8.6f %8.6f\n",sat[i],
                  sat[j],f<nf?"L":"P",fidx[f%nf+1],v[nv],Ri[nv],Rj[nv]);
                      ...
                      rtk_t *rtk, double *y)
{
    ...
    satposs(time,obsb,nb,nav,opt,opt->sateph,rs,dts,var,svh);
        ...
        for (f=0,k=na;f<nf;f++,k+=MAXSAT) {
            ...
            for (i=k;i<k+MAXSAT;i++) {
                if (rtk->x[i]==0.0||!test_sys(rtk->ssat[i-k].sys,m)||
                    !rtk->ssat[i-k].vsat[fidx[f]]||!rtk->ssat[i-k].half[fidx[f]]) {
                    continue;
                }
                if (rtk->ssat[i-k].lock[fidx[f]]>0&&!(rtk->ssat[i-k].slip[fidx[f]]&2)&&
                    rtk->ssat[i-k].azel[1]>=rtk->opt.elmaskar&&!nofix) {
                    rtk->ssat[i-k].fix[fidx[f]]=2; /* fix */
                    break;
                }
                else rtk->ssat[i-k].fix[fidx[f]]=1;
            }
            for (j=k;j<k+MAXSAT;j++) {
                if (i==j||rtk->x[j]==0.0||!test_sys(rtk->ssat[j-k].sys,m)||
                    !rtk->ssat[j-k].vsat[fidx[f]]) {
                    continue;
                }
                if (rtk->ssat[j-k].lock[fidx[f]]>0&&!(rtk->ssat[j-k].slip[fidx[f]]&2)&&
                    rtk->ssat[i-k].vsat[fidx[f]]&&
                    rtk->ssat[j-k].azel[1]>=rtk->opt.elmaskar&&!nofix) {
                    D[i+(na+nb)*nx]= 1.0;
                    D[j+(na+nb)*nx]=-1.0;
                    nb++;
                    rtk->ssat[j-k].fix[fidx[f]]=2; /* fix */
                }
                else rtk->ssat[j-k].fix[fidx[f]]=1;
            }
        }
    }
...
static void holdamb(rtk_t *rtk, const double *xa)
{
    ...
    for (m=0;m<5;m++) for (f=0;f<nf;f++) {
        ...
        for (n=i=0;i<MAXSAT;i++) {
            if (!test_sys(rtk->ssat[i].sys,m)||rtk->ssat[i].fix[fidx[f]]!=2||
                rtk->ssat[i].azel[1]<rtk->opt.elmaskhold) {
                continue;
            }
            index[n++]=IB(i+1,f,&rtk->opt);
			rtk->ssat[i].fix[fidx[f]] = 3; /* hold */
        }
                  ...
                  const nav_t *nav)
{
    ...
    satposs(time,obs,n,nav,opt,opt->sateph,rs,dts,var,svh);
    ...
    if (stat!=SOLQ_NONE&&zdres(0,obs,nu,rs,dts,var,svh,nav,xp,opt,0,y,e,azel)) {
        ...
        if (valpos(rtk,v,R,vflg,nv,4.0)) {
            ...
            for (i=0;i<ns;i++) for (f=0;f<nf;f++) {
                if (!rtk->ssat[sat[i]-1].vsat[fidx[f]]) continue;
				rtk->ssat[sat[i] - 1].lock[fidx[f]]++;
				rtk->ssat[sat[i] - 1].outc[fidx[f]] = 0;
				a = a < 0 ? f : a;
                if(a==f)rtk->sol.ns++; /* valid satellite count by L1 */
            }
    ...
    for (i=0;i<n;i++) for (j=0;j<nf;j++) {
        if (obs[i].L[fidx[j]]==0.0) continue;
        rtk->ssat[obs[i].sat-1].pt[obs[i].rcv-1][fidx[j]]=obs[i].time;
        rtk->ssat[obs[i].sat-1].ph[obs[i].rcv-1][fidx[j]]=obs[i].L[fidx[j]];
    }
    for (i=0;i<ns;i++) for (j=0;j<nf;j++) {
        
        /* output snr of rover receiver */
        rtk->ssat[sat[i]-1].snr[fidx[j]]=obs[iu[i]].SNR[fidx[j]];
    }
    for (i=0;i<MAXSAT;i++) for (j=0;j<nf;j++) {
        if (rtk->ssat[i].fix[fidx[j]]==2&&stat!=SOLQ_FIX) rtk->ssat[i].fix[fidx[j]]=1;
        if (rtk->ssat[i].slip[fidx[j]]&1) rtk->ssat[i].slipc[fidx[j]]++;
    }
    free(rs); free(dts); free(var); free(y); free(e); free(azel);
